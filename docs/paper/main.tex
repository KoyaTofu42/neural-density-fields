...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/vram_scaling.pdf}
\caption{\label{fig:vram}Memory scaling comparison between traditional 3D-CNN voxel grids and the PI-NDF continuous query architecture. While voxel memory explodes cubically ($O(N^3)$), PI-NDF maintains a flat memory footprint.}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/time_scaling.pdf}
\caption{\label{fig:time}Inference latency scaling for PI-NDF powered by \texttt{torch.compile}. Querying over half a million points takes only $\sim$145 milliseconds.}
\end{figure}
